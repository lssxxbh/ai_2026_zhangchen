...
class ParserService:
    ...
    async def parse_file(
        ...
        try:
            ...
            # 更新 ocr 字段中的文件类型信息
            if result and isinstance(result, dict) and "ocr" in result:
                result["ocr"]["file_type"] = file_type
            ...
            # 返回空结果结构
            empty_result = await self.ai_service.parse_report("")
            if empty_result and isinstance(empty_result, dict):
                empty_result["ocr"]["error"] = str(e)
                empty_result["ocr"]["file_type"] = file_type
            return empty_result
